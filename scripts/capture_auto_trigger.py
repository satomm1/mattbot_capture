#!/usr/bin/env python3
"""Start/stop capture sessions for navigation and person-detection triggers.
...
class CaptureAutoTrigger:
    def __init__(self):
        ...
        rospy.Subscriber(detections_topic, DetectedObjectArray, self._detected_objects_callback, queue_size=10)
        ...
        rospy.loginfo(
            "capture_auto_trigger ready nav_hz=%s person=%s person_hz=%s tail=%ss",
            self.nav_sample_hz,
    ...
    def _detected_objects_callback(self, msg: DetectedObjectArray):
        if not self.auto_capture_enabled or not self.person_capture_enabled or self._nav_active:
            return
