"""Local detection spool: SQLite chunks with hourly rotation."""
...
CREATE INDEX IF NOT EXISTS idx_detection_snapshots_wall_time ON detection_snapshots(wall_time);
"""
...
class DetectionSpoolError(Exception):
    pass


def detection_spool_root(spool_dir: str, robot_id: int) -> str:
...
class DetectionChunkWriter:
    ...
    def _open_db(self) -> None:
        ...
        self._conn.executescript(_CREATE_SNAPSHOTS)
...
def find_ready_detection_chunks(spool_dir: str, robot_id: int) -> list[str]:
    root = detection_spool_root(spool_dir, robot_id)
...
def detection_chunk_paths(spool_dir: str, robot_id: int, chunk_id: str) -> tuple[str, str]:
    root = detection_spool_root(spool_dir, robot_id)
    return (
...
def load_detection_chunk_meta(spool_dir: str, robot_id: int, chunk_id: str) -> dict:
    meta_path, _ = detection_chunk_paths(spool_dir, robot_id, chunk_id)
    with open(meta_path, encoding="utf-8") as handle:
        return json.load(handle)


def remove_detection_chunk(spool_dir: str, robot_id: int, chunk_id: str) -> None:
